...
pub trait MatrixActions<T> {
    ...
    fn nvals(&self) -> u64;
    fn clear(&mut self);
    fn assign_mxm<S: Semiring<T>>(&mut self, semiring: &S, a: &Self, b: &Self);
    ...
    fn assign_apply<X, O: UnaryOp<X, T>, M: Matrix<X>>(&mut self, op: &O, a: &M);
    ...
    fn assign_kronecker<S: Semiring<T>>(&mut self, semiring: &S, a: &Self, b: &Self);
...
impl<T, MT: Matrix<T>> MatrixActions<T> for MT {
    ...
    fn assign_mxm<S: Semiring<T>>(&mut self, semiring: &S, a: &Self, b: &Self) {
        grb_run!(
            GrB_mxm,
            self.grb_link_mut().link_mut(),
            ptr::null_mut(),
            ptr::null_mut(),
            semiring.grb_link().link(),
            a.grb_link().link(),
            b.grb_link().link(),
            ptr::null_mut()
        );
...
pub trait MatrixBuilder<T> {
    fn build<I: AsRef<[u64]>, J: AsRef<[u64]>, X: AsRef<[T]>, B: BinaryOp<T, T, T>>(
        &mut self,
        I: I,
        J: J,
        X: X,
    ...
    ( $rust_typ:ty, $grb_assign_fn:ident ) => {
        ...
        impl<M: Matrix<$rust_typ>> MatrixBuilder<$rust_typ> for M {
            fn build<
                ...
                J: AsRef<[u64]>,
                X: AsRef<[$rust_typ]>,
                B: BinaryOp<$rust_typ, $rust_typ, $rust_typ>,
            >(
                &mut self,
                I: I,
                J: J,
...
macro_rules! _matrix_insert_impl {
    ( $typ:ty, $set_elem_func:ident ) => {
        extern "C" {
            fn $set_elem_func(C: *mut c_void, x: $typ, i: u64, j: u64) -> u32;
        }
...
macro_rules! _matrix_get_impl {
    ( $typ:ty, $get_elem_func:ident ) => {
        extern "C" {
            fn $get_elem_func(x: *mut $typ, A: *const c_void, i: u64, j: u64) -> u32;
        }
...
#[link(name = "graphblas")]
extern "C" {
    fn GrB_Matrix_new(A: *mut *mut c_void, type_: *const c_void, nrows: u64, ncols: u64) -> u32;

    fn GrB_Matrix_dup(C: *mut *mut c_void, A: *const c_void) -> u32;

    fn GrB_Matrix_clear(A: *mut c_void) -> u32;

    fn GrB_Matrix_nrows(nrows: *mut u64, A: *const c_void) -> u32;

    fn GrB_Matrix_ncols(ncols: *mut u64, A: *const c_void) -> u32;

    fn GrB_Matrix_nvals(nvals: *mut u64, A: *const c_void) -> u32;

    fn GrB_Matrix_free(A: *mut *mut c_void) -> u32;
