...
static LIST: &[(&str, fn(&mut BaseTypeMatrix<bool>))] = &[
    ("square", Automaton::close_with_squaring),
    ("adj", Automaton::close_with_adjacency_matrix),
];
...
pub fn write_csv(path: String, csv_path: String, iterations: u64) -> Result<()> {
    ...
    for graph_dir in std::fs::read_dir(path)? {
        ...
        let graph = Automaton::read_graph(graph_dir.path().join(format!("{}.txt", graph_name)))?;
        for class_dir in std::fs::read_dir(graph_dir.path().join("queries"))? {
            ...
            for query_file in std::fs::read_dir(class_dir.path())? {
                ...
                let query = Automaton::read_query(query_file.path())?;
...
fn measure(graph: &Automaton, request: &Automaton) -> (u128, HashMap<String, (u128, u128, usize)>) {
    let mut map = HashMap::<String, (u128, u128, usize)>::new();

    let time = Instant::now();
    let intersection = graph.intersection(&request);
    ...
    for (name, func) in LIST {
        ...
        let time = Instant::now();
        let pairs = Automaton::extract_pairs(&closure, |_, _| true);
